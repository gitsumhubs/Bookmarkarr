...
Bookmarkarr is moving toward a layered backend where each project has a clear job:

- `bookmarkarr.domain` owns the domain model, value objects, domain exceptions, and business rules that do not need hosting, persistence, files, or network access.
- `bookmarkarr.application` owns use-case orchestration, application services, DTOs, mapping, and contracts that other layers implement. It can coordinate work, but it should avoid owning persistence, file, network, parsing, or image-processing implementations.
- `bookmarkarr.infrastructure` owns concrete adapters for technical concerns: EF Core and SQLite persistence, filesystem work, external HTTP clients, metadata/tagging libraries, HTML scraping/parsing, image inspection, cache implementations, SignalR infrastructure, and downloader integrations.
- `bookmarkarr.api` is the composition and hosting layer. It wires dependency injection, controllers, middleware, Swagger/OpenAPI, auth policy, and request pipeline behavior.
...
```text
bookmarkarr.domain/<Feature>/
bookmarkarr.application/<Feature>/{Contracts,Models,Services}/
bookmarkarr.infrastructure/<Feature>/{Persistence,Providers,Workers}/
bookmarkarr.api/Features/<Feature>/{Controllers,Models,Mapping}/
...
historical artifacts and remain under `Bookmarkarr.Infrastructure.Persistence.Migrations`.
...
The diagram describes the intended boundary: application is business/use-case logic and infrastructure is persistence, files, and external adapters. The codebase is still in transition, but implementation-specific packages should be kept out of `bookmarkarr.application` unless there is a documented reason to do otherwise.

New implementation-specific dependencies should go in `bookmarkarr.infrastructure`. The application layer should define contracts and coordinate use cases; infrastructure should implement those contracts with EF Core, filesystem, HTTP, parsing, image, tagging, and other adapter libraries.
...
- `bookmarkarr.application` no longer has an ASP.NET Core framework reference. It may reference general `Microsoft.Extensions.*` abstractions for logging, options, caching, dependency-factory access, and HTTP client factories, but it should not reference host/web implementation packages.

## API Startup Composition

`bookmarkarr.api/Program.cs` should stay as the host orchestration layer only: create the builder, register services, build the app, run startup tasks, initialize realtime logging, map the request pipeline, and run. Detailed startup behavior belongs in `bookmarkarr.api/Startup`.

Startup modules are grouped by concern:

- `BookmarkarrBuilderFactory` resolves content root/environment, external config, Serilog, default URLs, and the realtime log sink.
- `BookmarkarrServiceRegistration` coordinates service registration and owns API versioning setup.
- `BookmarkarrPlatformRegistration` owns reverse-proxy headers, development CORS, and SignalR JSON protocol setup.
- `BookmarkarrWorkflowRegistration` owns explicit application/API workflow registrations.
- `BookmarkarrSecurityStartup` owns antiforgery, Data Protection, and the security middleware order.
- `BookmarkarrStaticAssetsStartup` owns frontend static files, placeholder fallback, and cached-image static files.
- `BookmarkarrSwaggerRegistration`, `BookmarkarrStartupTasks`, and `BookmarkarrPipeline` own Swagger metadata, startup-time tasks, and the top-level request pipeline respectively.

Infrastructure-specific composition remains behind `bookmarkarr.infrastructure/Extensions/InfrastructureStartupCompositionExtensions.cs` and is invoked by `Program.cs`, the allowed composition root for cross-project infrastructure wiring.
...
The user-facing Activity queue should expose Bookmarkarr-owned active downloads only. Each active external queue item must first match a Bookmarkarr download by stored ID, client-specific ID, torrent hash, or a safe non-ambiguous title/artist fallback before it is shown. Unmatched active external items from shared clients such as Transmission, qBittorrent, SABnzbd, or NZBGet must be hidden from Activity so unrelated user transfers do not appear as Bookmarkarr work.
...
1. Keep the application-level interface, DTOs, and result models in `bookmarkarr.application` or `bookmarkarr.domain`.
2. Move the concrete implementation to the appropriate `bookmarkarr.infrastructure` feature or technology folder.
3. Register the implementation in `bookmarkarr.infrastructure/Extensions/InfrastructureServiceRegistrationExtensions.cs`.
4. Keep `bookmarkarr.api` responsible for calling the registration extension and composing the host.
