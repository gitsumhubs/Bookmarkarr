...
namespace Bookmarkarr.Application.Audiobooks.Catalog
{
    public class LibraryListService : ILibraryListService
    {
        ...
        public async Task<IReadOnlyList<LibraryAudiobookListItem>> GetAllAsync()
        {
            ...
            // scoped BookmarkarrDbContext, so they must be awaited sequentially: starting a second
            ...
            return audiobooks.Select(a =>
            {
                ...
                var wanted = a.Editions.Count > 0
                    ? a.Editions.Any(e => e.IsWanted)
                    : AudiobookWantedEvaluator.Compute(a.Monitored, hasTrackedFiles, a.FilePath);
                ...
                return new LibraryAudiobookListItem
                {
                    ...
                    Editions = a.Editions.Select(e => new LibraryEditionListItem
                    {
                        Id = e.Id,
