...
namespace Bookmarkarr.Application.Downloads.Processing
{
    /// <summary>
        ...
        TimeProvider timeProvider) : IDownloadProcessingJobService
    {
        ...
        public async Task<string> EnqueueAsync(Download download)
        {
            ...
            {
                throw new InvalidOperationException($"Trying to enqueue an import job for download {download.Id} with status {download.Status}: Status should be Completed or ImportPending");
            }
