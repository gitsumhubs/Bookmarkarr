...
namespace Bookmarkarr.Application.Downloads.Queue
{
        ...
        ILogger<DownloadQueueService> logger) : IDownloadQueueService
    {
        ...
        public async Task<QueueSnapshot> GetQueueSnapshotAsync()
        {
            ...
            var bookmarkarrDownloads = candidateSet.VisibleDownloads;
            ...
            queueItems.AddRange(bookmarkarrDownloads
            ...
            foreach (var clientQueueResult in clientQueueResults)
            {
                ...
                try
                {
                    ...
                    logger.LogInformation("Database has {DatabaseItems} Bookmarkarr downloads for metadata enrichment", bookmarkarrDownloads.Count);
                    ...
                    foreach (var queueItem in clientQueue)
                    {
                        ...
                        try
                        {
                            ...
                            if (matchedDownload != null)
                            {
                                ...
                                // Establish user-visible Bookmarkarr ownership before optional
                                // metadata persistence. If rebinding persistence fails, the
                                // catch below can still keep the matched Bookmarkarr item visible
                                // instead of leaking the raw external client row or hiding work
                                // that we already proved belongs to Bookmarkarr.
                                ...
                                queueItem.EditionId = matchedDownload.EditionId;
                                queueItem.MediaType = DownloadContentTypes.IsEbook(matchedDownload) ? "ebook" : "audiobook";
                            ...
                            else
                            {
                                ...
                                // Activity is Bookmarkarr's operational queue, not a mirror of every
                                ...
                                {
                                    logger.LogDebug(
                                        "Queue item {QueueId} '{Title}' from client {ClientName} is completed, not tracked by Bookmarkarr, and will be handled by completed external display",
                                        queueItem.Id,
                                        queueItem.Title,
                                        client.Name ?? client.Id);
                                }
                                ...
                                {
                                    logger.LogDebug(
                                        "Queue item {QueueId} '{Title}' from client {ClientName} is not tracked by Bookmarkarr and will be hidden from Activity",
                                        queueItem.Id,
                                        queueItem.Title,
                                        client.Name ?? client.Id);
                                }
                        ...
                        catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                        {
                            if (ownershipEstablished)
                            {
                                logger.LogWarning(
                                    ex,
                                    "Error enriching matched queue item {QueueId}; including matched Bookmarkarr-owned item",
                                    queueItem.Id);
                                mappedQueueItems.Add(queueItem);
                                continue;
                            }
            ...
            if (includeCompletedExternal)
            {
                try
                {
                    ...
                    var completedExternal = bookmarkarrDownloads
