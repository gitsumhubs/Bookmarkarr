...
namespace Bookmarkarr.Application.Downloads.Import
{
        ...
        ILogger<DownloadImportService> logger) : IDownloadImportService
    {
        ...
        {
            if (string.Equals(options?.ContentType, DownloadContentTypes.Ebook, StringComparison.OrdinalIgnoreCase))
            {
        ...
        private async Task<List<ImportResult>> ImportEbookFilesAsync(
            Audiobook audiobook,
            List<string> files,
            DownloadImportOptions options,
            CancellationToken ct)
        {
            var ebookRoot = string.IsNullOrWhiteSpace(options.LibraryRoot)
                ? "/ebooks"
            ...
            try
            {
                ...
                files = [.. files.Where(file => !archives.Contains(file, StringComparer.OrdinalIgnoreCase))];
                if (archives.Count > 0 && (settings.ExtractArchives || options.ForceArchiveExtraction))
                {
                    files.AddRange(await archiveImportExtractor.ExtractAsync(archives));
                    if (completedFileAction == FileAction.HardlinkCopy)
                    {
                        completedFileAction = FileAction.Copy;
                        logger.LogWarning(
                            "Ebook download for book {AudiobookId} contains archives; completed action switched from hardlink to copy",
                            audiobook.Id);
                    }
                }
                ...
                var author = SanitizeEbookPathSegment(
                    audiobook.Authors?.FirstOrDefault(a => !string.IsNullOrWhiteSpace(a)) ?? "Unknown Author");
                var title = SanitizeEbookPathSegment(audiobook.Title ?? "Unknown Title");
                var series = SanitizeEbookPathSegment(audiobook.Series ?? string.Empty);
                var relativeFolder = string.IsNullOrWhiteSpace(series)
                    ? Path.Join(author, title)
                    : Path.Join(author, series, title);

                if (!destinationPlanner.TryResolve(ebookRoot, relativeFolder, out var destinationFolder))
                {
                    return
                    [
                        ImportResult.ImportFailure(
                ...
                foreach (var file in ebookFiles)
                {
                    ...
                    var filename = title + Path.GetExtension(file).ToLowerInvariant();
                    if (!destinationPlanner.TryResolve(destinationFolder, filename, out var destination))
                    {
                        results.Add(ImportResult.ImportFailure(completedFileAction, file, destinationFolder));
                        continue;
                    }
                    ...
                    destination = await destinationPlanner.ResolveIdempotentOrUniqueAsync(
                        ...
                        usedDestinations,
                        ct);
                    var alreadyPresent = await destinationPlanner.IsExistingEquivalentAsync(file, destination, ct);
                    if (!(alreadyPresent && completedFileAction != FileAction.Move)
                        && !await fileMover.PerformActionOn(completedFileAction, file, destination))
                    {
                        results.Add(ImportResult.ImportFailure(completedFileAction, file, destination));
                        continue;
