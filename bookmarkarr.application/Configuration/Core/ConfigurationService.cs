...
namespace Bookmarkarr.Application.Configuration.Core
{
    ...
    {
        private static readonly SemaphoreSlim SettingsInitializationLock = new(1, 1);
        ...
        {
            await SettingsInitializationLock.WaitAsync();
            try
            {
                return await LoadApplicationSettingsAsync();
            }
            finally
            {
                SettingsInitializationLock.Release();
            }
        }

        private async Task<ApplicationSettings> LoadApplicationSettingsAsync()
        {
            try
            {
