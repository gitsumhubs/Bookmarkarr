...
namespace Bookmarkarr.Application.Configuration.Core
{
    public class StartupConfigService : IStartupConfigService
    {
        ...
        public StartupConfigService(ILogger<StartupConfigService> logger, IApplicationPathService applicationPathService, IFileSystem fileSystem)
        {
            ...
            // <repoRoot>/bookmarkarr.api/config/config.json for local development
            ...
            if (isDev && !isDocker)
            {
                try
                {
                    // Search upward from the current working directory for bookmarkarr.sln
                    ...
                    try
                    {
                        ...
                        {
                            if (_fileSystem.FileExists(Path.Join(dir, "bookmarkarr.sln")))
                            {
                                repoRootFromCwd = dir;
                                break;
                            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    // config at <ancestor>/bookmarkarr.api/config/config.json and prefer that.
                    if (string.IsNullOrEmpty(_configPath))
                    {
                        ...
                        while (!string.IsNullOrWhiteSpace(dirInfo) && depth++ < maxDepth)
                        {
                            var candidateConfigPath = Path.Join(dirInfo, "bookmarkarr.api", "config", "config.json");
        ...
        {
            var config = _config ?? new StartupConfig();
            var apiKey = Environment.GetEnvironmentVariable("BOOKMARKARR_API_KEY");
            if (!string.IsNullOrWhiteSpace(apiKey)) config.ApiKey = apiKey;
            var auth = Environment.GetEnvironmentVariable("BOOKMARKARR_AUTHENTICATION_REQUIRED");
            if (!string.IsNullOrWhiteSpace(auth)) config.AuthenticationRequired = auth;
            return config;
        }

        public bool IsAuthenticationRequired()
            => (GetConfig() ?? new StartupConfig()).IsAuthenticationEnabled();
