...
namespace Bookmarkarr.Infrastructure.DownloadClients.Nzbget
{
        ...
        ILogger<NzbgetAdapter> logger)
    {
        public async Task<List<QueueItem>> GetQueueAsync(DownloadClientConfiguration client, List<string> ids, CancellationToken ct = default)
        {
            ...
            if (isMonitorPoll && !string.IsNullOrWhiteSpace(configuredCategory))
            {
                // Monitor both Bookmarkarr media categories without admitting unrelated
                // queue/history content from the same externally managed client. When
                // the client has no configured category, retain the upstream unfiltered
                // behavior; the monitored ID set still prevents unrelated entries from
                // being returned to the caller.
                configuredCategory = string.Join('|', new[]
                {
                    configuredCategory,
                    Environment.GetEnvironmentVariable("BOOKMARKARR_AUDIOBOOK_CATEGORY") ?? "audiobooks",
                    Environment.GetEnvironmentVariable("BOOKMARKARR_EBOOK_CATEGORY") ?? "ebooks"
