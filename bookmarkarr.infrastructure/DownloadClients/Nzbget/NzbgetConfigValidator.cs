...
internal static class NzbgetConfigValidator
{
    public static (bool Success, string Message) ValidateKeepHistory(XElement configResult)
    {
        ...
        if (keepHistory == null)
        {
            return (
                false,
                "NZBGet: KeepHistory setting was not found. Bookmarkarr requires NZBGet history so completed downloads can be imported reliably.");
        }
        ...
        if (!int.TryParse(normalizedKeepHistory, NumberStyles.None, CultureInfo.InvariantCulture, out var value))
        {
            return (
                false,
                "NZBGet: KeepHistory setting is invalid. Bookmarkarr requires KeepHistory to be greater than 0.");
        }
        ...
        if (value <= 0)
        {
            return (
                false,
                "NZBGet: KeepHistory must be greater than 0 so Bookmarkarr can read completed download history and resolve import paths.");
        }
