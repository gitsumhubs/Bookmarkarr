        ...
        string clientType)
    {
        public async Task<List<QueueItem>> GetQueueAsync(DownloadClientConfiguration client, List<string> ids, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // ID-filtered monitor polls must see tracked ebook submissions even
                // when the saved client category is the audiobook default.
                var categoryFilter = isMonitorPoll
                    ? string.Empty
                    : QBittorrentHelpers.BuildCategoryParameter(client.Settings, "&");
