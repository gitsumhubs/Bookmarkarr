...
namespace Bookmarkarr.Infrastructure.Notifications.Discord
{
    public class DiscordBotService : IDiscordBotService
    {
        ...
        public async Task<bool> StartBotAsync()
        {
            ...
            try
            {
                ...
                // Determine the Bookmarkarr API URL with proper fallbacks for Docker
                var bookmarkarrUrl = GetBookmarkarrUrl();

                _logger.LogInformation("Starting Discord bot with BOOKMARKARR_URL: {Url}", bookmarkarrUrl);
                startInfo.EnvironmentVariables["BOOKMARKARR_URL"] = bookmarkarrUrl;
                ...
                try
                {
                    ...
                    if (cfg != null && !string.IsNullOrWhiteSpace(cfg.ApiKey))
                    {
                        ...
                        startInfo.EnvironmentVariables["BOOKMARKARR_API_KEY"] = _botApiKey;
                        _logger.LogInformation("Passing BOOKMARKARR_API_KEY to bot process (present=true)");
                    }
                }
        ...
        private string GetBookmarkarrUrl()
        {
            // Priority 1: BOOKMARKARR_PUBLIC_URL environment variable (Docker deployments)
            var envUrl = Environment.GetEnvironmentVariable("BOOKMARKARR_PUBLIC_URL");
            if (!string.IsNullOrWhiteSpace(envUrl))
            {
                _logger.LogInformation("Using BOOKMARKARR_PUBLIC_URL from environment: {Url}", envUrl);
