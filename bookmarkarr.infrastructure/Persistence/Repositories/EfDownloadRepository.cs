...
namespace Bookmarkarr.Infrastructure.Persistence.Repositories
{
    ...
    {
        private readonly IDbContextFactory<BookmarkarrDbContext> _dbFactory;
        private readonly ILogger<EfDownloadRepository> _logger;

        public EfDownloadRepository(IDbContextFactory<BookmarkarrDbContext> dbFactory, ILogger<EfDownloadRepository> logger)
        {
        ...
        private static void ApplyActiveDeduplicationKey(Download download)
        {
            download.ActiveAudiobookDeduplicationKey =
                IsActive(download.Status)
                    ? download.EditionId ?? (DownloadContentTypes.IsEbook(download) ? null : download.AudiobookId)
                    : null;
        }
