...
namespace Bookmarkarr.Infrastructure.Persistence.Repositories
{
    ...
    {
        private static readonly SemaphoreSlim SingletonSaveLock = new(1, 1);
        private readonly BookmarkarrDbContext _db;

        public EfApplicationSettingsRepository(BookmarkarrDbContext db)
        {
        ...
        {
            await SingletonSaveLock.WaitAsync(ct);
            try
            {
                return await SaveCoreAsync(settings, ct);
            }
            finally
            {
                SingletonSaveLock.Release();
            }
        }

        private async Task<ApplicationSettings> SaveCoreAsync(ApplicationSettings settings, CancellationToken ct)
        {
            settings.Id = 1;
            ...
            if (existing == null)
            {
                ...
                {
                    // Fresh-start hosted services can race to create singleton row 1.
                    // Treat the winning insert as the canonical idempotent result.
                    _db.Entry(settings).State = EntityState.Detached;
                    return await _db.ApplicationSettings.AsNoTracking().SingleAsync(item => item.Id == 1, ct);
                }
            }
