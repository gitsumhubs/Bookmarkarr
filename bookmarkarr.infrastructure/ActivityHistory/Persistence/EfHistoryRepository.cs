...
namespace Bookmarkarr.Infrastructure.ActivityHistory.Persistence
{
    public class EfHistoryRepository : IHistoryRepository
    {
        private readonly BookmarkarrDbContext _db;

        public EfHistoryRepository(BookmarkarrDbContext db)
        {
        ...
        public async Task<HistoryPage> QueryAsync(HistoryQuery query, CancellationToken ct = default)
        {
            ...
            if (query.EditionId.HasValue)
                filtered = filtered.Where(h => h.EditionId == query.EditionId.Value);
            if (!string.IsNullOrWhiteSpace(query.MediaType))
                filtered = filtered.Where(h => h.MediaType == query.MediaType.ToLower());
