    ...
    public static class AudiobookIdentifierNormalizer
    {
            ...
            out string? error)
        {
            ...
            switch (type)
            {
                ...
                case AudiobookExternalIdentifierType.GoodreadsId:
                    {
                        var candidate = new string(trimmed.Where(char.IsDigit).ToArray());
                        if (candidate.Length == 0 || candidate.Length > 32)
                        {
                            error = "Goodreads ID must contain 1 to 32 digits.";
                            return false;
                        }
                        normalizedValue = candidate;
                        return true;
