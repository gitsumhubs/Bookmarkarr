...
export interface QueueItem {
  ...
  localPath?: string // Path translated for Bookmarkarr
}
...
export type AudiobookExternalIdentifierType = 'Asin' | 'Isbn' | 'OpenLibraryId' | 'GoodreadsId'
...
export interface Audiobook {
  ...
  editions?: {
    ...
    mediaType: EditionMediaType
    monitored: boolean
    status: 'Unmonitored' | 'Missing' | 'Queued' | 'Downloading' | 'Imported' | 'UpgradeAvailable'
    wanted: boolean
    qualityProfileId?: number
    rootFolderId?: number
    rootPath?: string
...
export type EditionMediaType = 'Audiobook' | 'Ebook'

export interface GoodreadsMatchCandidate {
  bookId: number
  title: string
...
export interface GoodreadsPreviewRow {
  ...
  selected: boolean
  mediaFormats: EditionMediaType[]
  matchStatus: 'new' | 'matched' | 'ambiguous'
  matchedBookId?: number
  matchMethod?: string
  matchCandidates: GoodreadsMatchCandidate[]
  resolvedBookId?: number
}

export interface GoodreadsPreviewResponse {
  batchId: string
  expiresAt: string
...
export interface GoodreadsCommitSummary {
  batchId: string
  createdBooks: number
