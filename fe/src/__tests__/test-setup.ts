...
vi.mock('@/services/api', () => {
  const apiService = {
    ...
    searchBookEdition: vi.fn(async () => ({ editionId: 1, mediaType: 'Ebook', results: [] })),
    updateBookEdition: vi.fn(async () => ({})),
    previewGoodreadsImport: vi.fn(async () => ({
      batchId: 'batch-test',
    ...
    getGoodreadsImport: vi.fn(async () => ({ rows: [] })),
    commitGoodreadsImport: vi.fn(async () => ({
      batchId: 'batch-test',
      createdBooks: 0,
      createdEditions: 0,
      unchangedRows: 0,
