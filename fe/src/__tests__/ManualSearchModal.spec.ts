...
describe('ManualSearchModal.vue', () => {
  ...
  it('defaults to audiobooks and searches again in ebook-only mode', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.findAll('.content-mode-button')).toHaveLength(2))
    const ebookButton = wrapper.findAll('.content-mode-button')[1]!
    expect(ebookButton.attributes('disabled')).toBeUndefined()
    await ebookButton.trigger('click')

    await vi.waitFor(() => expect(searchByApi).toHaveBeenCalledTimes(2))
    expect(searchByApi.mock.calls[1]?.[3]).toMatchObject({ contentType: 'ebook' })
  })
