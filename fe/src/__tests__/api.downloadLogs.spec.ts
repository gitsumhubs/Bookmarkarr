...
describe('ApiService.downloadLogs', () => {
  ...
  it('downloads logs through cookie-authenticated fetch when session auth is enabled', async () => {
    ...
    const createObjectUrlSpy = vi
      ...
      .mockReturnValue('blob:bookmarkarr-download')
      ...
      return new Response(
        ...
        }),
        {
          ...
          headers: {
            ...
            'Content-Disposition': 'attachment; filename="bookmarkarr-test.log"',
          },
        },
      )
    })
    ...
    expect(anchor.download).toBe('bookmarkarr-test.log')
    expect(anchor.href).toBe('blob:bookmarkarr-download')
    expect(appendSpy).toHaveBeenCalledTimes(1)
    expect(removeSpy).toHaveBeenCalledTimes(1)
    expect(revokeObjectUrlSpy).toHaveBeenCalledWith('blob:bookmarkarr-download')
  })
})
