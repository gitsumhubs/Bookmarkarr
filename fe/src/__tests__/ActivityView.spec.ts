...
describe('ActivityView', () => {
  ...
  it('offers Bookmarkarr-only removal when an external item is no longer in the client queue', async () => {
    mockSignalR()
    const apiService = mockApi({
      getQueue: vi.fn(async () => []),
    })
  ...
  it('removes a failed DDL download through Bookmarkarr cancellation', async () => {
    mockSignalR()
    const apiService = mockApi()
    mockConfigurationStore(false)
    mockLibraryStore()
