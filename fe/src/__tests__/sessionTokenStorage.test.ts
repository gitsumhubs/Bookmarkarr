...
describe('sessionTokenManager storage propagation', () => {
  beforeEach(() => {
    ...
    try {
      ...
      localStorage.removeItem('bookmarkarr_session_token')
      localStorage.removeItem('bookmarkarr_session_event')
      localStorage.removeItem('bookmarkarr_session_token_persistence')
      sessionStorage.removeItem('bookmarkarr_session_token')
    } catch {}
  })
  ...
  it('notifies subscribers when storage is changed (cross-tab)', () => {
    ...
    localStorage.setItem('bookmarkarr_session_token', token)
    // Emit storage event
    const ev = new Event('storage')
    Object.defineProperty(ev, 'key', { value: 'bookmarkarr_session_token' })
    ...
    Object.defineProperty(ev2, 'key', { value: 'bookmarkarr_session_token' })
  ...
  it('sanitizes stored browser auth state so bearer secrets are not persisted', () => {
    ...
    expect(localStorage.getItem('bookmarkarr_session_token')).toBe('cookie-session')
  })
  ...
  it('reacts to cross-tab auth event broadcasts without storing a secret', () => {
    ...
    Object.defineProperty(loginEvent, 'key', { value: 'bookmarkarr_session_event' })
    ...
    Object.defineProperty(logoutEvent, 'key', { value: 'bookmarkarr_session_event' })
