...
export const logSessionState = (context: string = 'Unknown') => {
  try {
    ...
    const storedToken = localStorage.getItem('bookmarkarr_session_token')
    ...
    try {
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (key && (key.includes('auth') || key.includes('session') || key.includes('bookmarkarr'))) {
...
export const clearAllAuthData = () => {
  try {
    ...
    try {
      ...
      sessionStorage.removeItem('bookmarkarr_pending_redirect')
