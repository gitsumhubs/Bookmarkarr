            ...
            >
              ...
              <div class="day-items">
                <div
                  ...
                  :key="item.key"
                  class="calendar-item"
                  :title="`${item.displayTitle}${item.author ? ' - ' + item.author : ''}`"
                  role="button"
                  tabindex="0"
          ...
          <div class="week-body">
            <div v-for="day in weekDates" :key="day.date" class="week-day-column">
              <div
                ...
                @keydown.space.prevent="navigateToDetail(item.id)"
              >
                <div class="week-item-title">{{ item.displayTitle }}</div>
...
const calendarStorageKey = 'bookmarkarr.calendar.currentDate'
const viewModes = [
...
const calendarItems = computed<CalendarItem[]>(() => {
  return libraryStore.audiobooks
    .flatMap((book) => {
      ...
      if (!key) return []
      const editions = book.editions?.filter((edition) => edition.monitored) ?? []
      const base = {
          id: book.id,
      ...
      if (!editions.length) return [{ ...base, key: `book-${book.id}`, displayTitle: base.title }]
      return editions.map((edition) => ({
        ...base,
        key: `edition-${edition.id}`,
        editionId: edition.id,
        mediaType: edition.mediaType,
        displayTitle: `${base.title} · ${edition.mediaType === 'Audiobook' ? 'Audio' : 'Ebook'}`,
      }))
    })
})
