...
if [ "$(id -u)" = "0" ] && { [ "$PUID" != "0" ] || [ "$PGID" != "0" ]; }; then
    echo "Starting Bookmarkarr with UID=$PUID GID=$PGID UMASK=$UMASK"

    if [ "$PUID" != "0" ] && [ "$PGID" != "0" ]; then
        groupmod -o -g "$PGID" bookmarkarr
        usermod -o -u "$PUID" bookmarkarr
        chown -R "$PUID:$PGID" /app/config

        exec gosu bookmarkarr dotnet Bookmarkarr.Api.dll "$@"
    fi

    chown -R "$PUID:$PGID" /app/config
    exec gosu "$PUID:$PGID" dotnet Bookmarkarr.Api.dll "$@"
fi

echo "Starting Bookmarkarr as $(id)"
exec dotnet Bookmarkarr.Api.dll "$@"
