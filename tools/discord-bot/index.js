/*
  Simple Discord bot to integrate with Bookmarkarr API.
  - Reads Bookmarkarr settings from GET /api/v{version}/configuration/settings
  ...
  Usage: (from repo root)
    ...
    npm install
    BOOKMARKARR_URL=http://localhost:4545 node index.js
...
const BOOKMARKARR_API_KEY = process.env.BOOKMARKARR_API_KEY || null
if (BOOKMARKARR_API_KEY) {
  ...
  fetch = async (url, opts) => {
    ...
    if (!opts.headers['X-Api-Key'] && !opts.headers['x-api-key']) {
      opts.headers['X-Api-Key'] = BOOKMARKARR_API_KEY
    }
    return rawFetch(url, opts)
  }
  console.log('Configured bot to use BOOKMARKARR_API_KEY for backend requests')
}
...
// Determine Bookmarkarr base URL with several fallbacks:
// 1) process.env.BOOKMARKARR_URL
// 2) tools/discord-bot/.env (key BOOKMARKARR_URL)
...
function readLocalEnvFile(envPath) {
  try {
    ...
    for (const line of lines) {
      const m = line.match(/^\s*BOOKMARKARR_URL\s*=\s*(.+)\s*$/)
...
function writeLocalEnvFile(envPath, url) {
  try {
    const content = `BOOKMARKARR_URL=${url}\n`
    fs.writeFileSync(envPath, content, { encoding: 'utf8', flag: 'w' })
    console.log(`Saved BOOKMARKARR_URL to ${envPath}`)
  } catch (e) {
    console.warn('Failed to write .env file for bookmarkarr url:', e && e.message ? e.message : e)
  }
}

function promptForBookmarkarrUrl(defaultUrl) {
  try {
    if (!process.stdin.isTTY) return defaultUrl
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout })
    return new Promise(resolve => {
      rl.question(`Enter Bookmarkarr URL [${defaultUrl}]: `, answer => {
        rl.close()
...
async function resolveBookmarkarrUrl() {
  // env var takes precedence
  if (process.env.BOOKMARKARR_URL && process.env.BOOKMARKARR_URL.trim()) return process.env.BOOKMARKARR_URL.trim()
  const envPath = path.join(__dirname, '.env')
  const local = readLocalEnvFile(envPath)
  if (local) return local
  // prompt interactively and persist
  const defaultUrl = 'http://localhost:4545'
  const chosen = await promptForBookmarkarrUrl(defaultUrl)
...
// bookmarkarrUrl will be resolved at startup (may prompt once and write tools/discord-bot/.env)
let bookmarkarrUrl = 'http://localhost:4545'
resolveBookmarkarrUrl().then(u => { bookmarkarrUrl = (u || bookmarkarrUrl).replace(/\/$/, '') }).catch(() => {})
let bookmarkarrApiVersion = normalizeApiVersion(process.env.BOOKMARKARR_API_VERSION || '1')
...
function applyApiVersionFromPayload(payload) {
  try {
    ...
    if (!next || next === bookmarkarrApiVersion) return
    bookmarkarrApiVersion = next
    console.log(`Discord bot API version updated to v${bookmarkarrApiVersion}`)
  } catch {}
}

function buildApiUrl(endpoint) {
  const raw = String(endpoint || '').trim()
  if (!raw) return `${bookmarkarrUrl.replace(/\/$/, '')}/api/v${bookmarkarrApiVersion}`
  if (/^https?:\/\//i.test(raw)) return raw
  const withSlash = raw.startsWith('/') ? raw : `/${raw}`
  const normalized = withSlash.replace(/^\/api(?:\/v\d+(?:\.\d+)?)?/i, '')
  return `${bookmarkarrUrl.replace(/\/$/, '')}/api/v${bookmarkarrApiVersion}${normalized}`
}
...
async function createSignalRConnection() {
  ...
  const hubUrl = `${bookmarkarrUrl.replace(/\/$/, '')}/hubs/settings`
  ...
  if (BOOKMARKARR_API_KEY) {
    hubBuilder.withUrl(hubUrl, { accessTokenFactory: () => BOOKMARKARR_API_KEY })
  } else {
  ...
  client.on('interactionCreate', async (interaction) => {
    try {
      if (interaction.type === InteractionType.ApplicationCommand) {
        ...
        if (interaction.customId.startsWith('bookmarkarr_results_')) {
...
async function handleSearchCommand(interaction, title, author) {
  // If a channel is configured in Bookmarkarr settings, only allow commands from that channel
  ...
  // Call Bookmarkarr search by title
  try {
  ...
  logSessionEvent(`Created confirm session ${confirmSessionId} title="${(md.title||'').replace(/\n/g,' ')}" (single-result)`)
    ...
    const select = new StringSelectMenuBuilder()
      .setCustomId(`bookmarkarr_results_${makeSessionId()}`)
...
async function start() {
  console.log('Starting Bookmarkarr Discord bot (tools/discord-bot)')
  // Resolve the Bookmarkarr URL synchronously from env/.env before making any requests.
  ...
  try {
    const resolved = await resolveBookmarkarrUrl()
    if (resolved && resolved.trim()) bookmarkarrUrl = resolved.trim().replace(/\/$/, '')
  } catch (e) {
    // ignore and fall back to existing value
  }

  // initial fetch (now guaranteed to use the resolved bookmarkarrUrl)
