...
namespace Bookmarkarr.Tests.Features.Infrastructure.Library.Moving
{
    public class MoveBackgroundService_FailureTests : BaseTests
    {
        ...
        public async Task MoveBackgroundService_Fails_WhenFileLocked_IncrementsAttemptCount()
        {
            ...
            var src = FileService.GetTempDirectory("bookmarkarr_test_src_lock");
            var file = await FileService.GetFileAsync(src, "file_locked.txt", "locked");

            var dst = await FileService.GetFileAsync(FileService.GetTempPath(), "bookmarkarr_test_dst_lock", "block");
