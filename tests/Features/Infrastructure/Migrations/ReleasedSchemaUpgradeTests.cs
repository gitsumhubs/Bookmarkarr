...
namespace Bookmarkarr.Tests.Features.Infrastructure.Migrations;
...
public sealed class ReleasedSchemaUpgradeTests
{
    ...
    public async Task PreviousReleasedSchema_UpgradesToCurrentModel()
    {
        ...
        try
        {
            await using (var db = new BookmarkarrDbContext(options))
            {
            ...
            await using var verified = new BookmarkarrDbContext(options);
