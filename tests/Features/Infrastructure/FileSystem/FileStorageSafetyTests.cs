...
namespace Bookmarkarr.Tests.Features.Infrastructure.FileSystem
{
    public class FileStorageSafetyTests : IDisposable
    {
        ...
        {
            _root = Path.Join(Path.GetTempPath(), "bookmarkarr-storage-root-" + Guid.NewGuid().ToString("N"));
            _outside = Path.Join(Path.GetTempPath(), "bookmarkarr-storage-outside-" + Guid.NewGuid().ToString("N"));
