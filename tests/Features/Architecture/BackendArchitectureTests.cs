...
namespace Bookmarkarr.Tests.Features.Architecture;
...
public sealed class BackendArchitectureTests
{
    ...
    public void DomainAndApplication_DoNotReferenceImplementationProjects()
    {
        AssertProjectReferences(
            "bookmarkarr.domain/Bookmarkarr.Domain.csproj",
            []);
        AssertProjectReferences(
            "bookmarkarr.application/Bookmarkarr.Application.csproj",
            ["../bookmarkarr.domain/Bookmarkarr.Domain.csproj"]);
    }
    ...
    public void DomainAndApplication_DoNotReferenceForbiddenImplementationPackages()
    {
        ...
        AssertNoPackages("bookmarkarr.domain/Bookmarkarr.Domain.csproj", forbiddenPackages);
        AssertNoPackages("bookmarkarr.application/Bookmarkarr.Application.csproj", forbiddenPackages);
    }
    ...
    {
        AssertNoPackages(
            "bookmarkarr.api/Bookmarkarr.Api.csproj",
            [
    ...
    {
        var applicationRoot = Path.Join(RepositoryRoot, "bookmarkarr.application");
    ...
    public void NewDirectHttpClientConstruction_IsRestrictedToDocumentedLegacyAdapters()
    {
        ...
        {
            "bookmarkarr.application/Common/MyAnonamouseHelper.cs",
            "bookmarkarr.api/Features/Indexers/IndexerDebugSearchWorkflow.cs",
            "bookmarkarr.infrastructure/DownloadClients/Qbittorrent/QbittorrentCookieSession.cs",
            "bookmarkarr.infrastructure/DownloadClients/Transmission/TransmissionAdapter.cs",
            "bookmarkarr.infrastructure/Torrents/TorrentFileDownloader.cs"
        };
        var roots = new[]
        {
            "bookmarkarr.application",
            "bookmarkarr.infrastructure",
            "bookmarkarr.api"
        };
    ...
    public void NewControllerBroadCatches_AreForbiddenOutsideDocumentedLegacyControllers()
    {
        ...
        var apiRoot = Path.Join(RepositoryRoot, "bookmarkarr.api");
    ...
    {
        AssertNamespacesMatchFolders("bookmarkarr.domain", "Bookmarkarr.Domain");
        AssertNamespacesMatchFolders("bookmarkarr.application", "Bookmarkarr.Application");
        AssertNamespacesMatchFolders("bookmarkarr.infrastructure", "Bookmarkarr.Infrastructure");
        AssertNamespacesMatchFolders("bookmarkarr.api", "Bookmarkarr.Api");
    }
    ...
    public void EfMigrations_RemainInTheirHistoricalNamespace()
    {
        ...
        foreach (var file in Directory.EnumerateFiles(migrationRoot, "*.cs", SearchOption.AllDirectories))
        {
            ...
            var expectedNamespace = relativeDirectory == "."
                ? "Bookmarkarr.Infrastructure.Persistence.Migrations"
                : $"Bookmarkarr.Infrastructure.Persistence.Migrations.{ToNamespace(relativeDirectory)}";
    ...
    {
        var adapterRoot = Path.Join(RepositoryRoot, "bookmarkarr.infrastructure", "DownloadClients");
    ...
    {
        var domainRoot = Path.Join(RepositoryRoot, "bookmarkarr.domain");
        var filesystemPattern = new Regex(
    ...
    public void ExternalHttpClients_HaveOneRegistrationOwner()
    {
        ...
        {
            Path.Join(RepositoryRoot, "bookmarkarr.api", "Startup", "BookmarkarrWorkflowRegistration.cs"),
            Path.Join(RepositoryRoot, "bookmarkarr.infrastructure", "DependencyInjection", "Platform", "PlatformRegistrationExtensions.cs"),
            Path.Join(RepositoryRoot, "bookmarkarr.infrastructure", "DependencyInjection", "Metadata", "MetadataRegistrationExtensions.cs"),
            Path.Join(RepositoryRoot, "bookmarkarr.infrastructure", "DependencyInjection", "InfrastructureStartupCompositionExtensions.cs")
        };
    ...
    public void Controllers_DoNotResolveServicesOrImplementPersistence()
    {
        var controllerFiles = Directory
            .EnumerateFiles(
                Path.Join(RepositoryRoot, "bookmarkarr.api", "Features"),
                "*Controller.cs",
                SearchOption.AllDirectories)
            .Where(file => !IsBuildArtifact(file))
            .ToList();
        ...
        var violations = controllerFiles
            ...
            .Select(file => Normalize(Path.GetRelativePath(
                Path.Join(RepositoryRoot, "bookmarkarr.api"),
                file)))
            .ToList();
    ...
    private static string FindRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Join(directory.FullName, "bookmarkarr.slnx")))
            {
