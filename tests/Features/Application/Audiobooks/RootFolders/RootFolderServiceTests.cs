...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.RootFolders
{
    public class RootFolderServiceTests
    {
        ...
        public async Task Update_RenameWithoutMove_UpdatesAudiobookBasePaths()
        {
            ...
            using (var pre = new BookmarkarrDbContext(options))
            {
            ...
            using (var verifyDb = new BookmarkarrDbContext(options))
            {
        ...
        private class TestDbFactory : IDbContextFactory<BookmarkarrDbContext>
        {
            private readonly DbContextOptions<BookmarkarrDbContext> _options;
            public TestDbFactory(DbContextOptions<BookmarkarrDbContext> options) { _options = options; }
            public Task<BookmarkarrDbContext> CreateDbContextAsync() => Task.FromResult(new BookmarkarrDbContext(_options));
            public BookmarkarrDbContext CreateDbContext() => new BookmarkarrDbContext(_options);
        }
