...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.Files
{
    public class AudioFileServiceTests : BaseTests
    {
        ...
        public async Task EnsureAudiobookFileAsync_HandlesUniqueConstraintViolation_ReturnsFalse()
        {
            ...
            // subclass BookmarkarrDbContext to override SaveChangesAsync
        ...
        {
            var oldDir = Path.Join(Path.GetTempPath(), "bookmarkarr-audiofile-old", Guid.NewGuid().ToString(), "Old Folder");
            Directory.CreateDirectory(oldDir);
            var oldFile = Path.Join(oldDir, "track1.m4b");
            await File.WriteAllTextAsync(oldFile, "old");

            var importDir = Path.Join(Path.GetTempPath(), "bookmarkarr-audiofile-new", Guid.NewGuid().ToString(), "Jack of Shadows");
        ...
        private class ThrowingSaveChangesDbContext : BookmarkarrDbContext
        {
            public ThrowingSaveChangesDbContext(DbContextOptions<BookmarkarrDbContext> options) : base(options) { }
