...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.Quality
{
    public class QualityProfileScoringTests
    {
        ...
        public async Task Age_Rejection_Is_Skipped_When_IndexerRetention_Is_Larger()
        {
            ...
            var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<BookmarkarrDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options;
            using var db = new BookmarkarrDbContext(options);
            var indexer = new Bookmarkarr.Domain.Search.Indexer { Name = "TestIndexer", Url = "https://test.local", Retention = 30, IsEnabled = true };
        ...
        public async Task Age_Rejection_Applied_When_Age_Exceeds_IndexerRetention()
        {
            ...
            var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<BookmarkarrDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options;
            using var db = new BookmarkarrDbContext(options);
            var indexer = new Bookmarkarr.Domain.Search.Indexer { Name = "TestIndexer2", Url = "https://test2.local", Retention = 10, IsEnabled = true };
            db.Indexers.Add(indexer);
            db.SaveChanges();
        ...
        public async Task Torrent_Age_Rejection_Uses_ProfileMaxAge()
        {
            ...
            var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<BookmarkarrDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options;
            using var db = new BookmarkarrDbContext(options);
            var idx = new Bookmarkarr.Domain.Search.Indexer { Name = "TorrentIdx", Url = "https://t.local", IsEnabled = true };
            db.Indexers.Add(idx);
            db.SaveChanges();
