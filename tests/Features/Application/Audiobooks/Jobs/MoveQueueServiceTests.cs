...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.Jobs
{
    public class MoveQueueServiceTests
    {
        ...
        {
            var dbOpts = new DbContextOptionsBuilder<BookmarkarrDbContext>()
                .UseInMemoryDatabase("test_db_movejob_" + Guid.NewGuid().ToString("N"))
                .Options;
            var db = new BookmarkarrDbContext(dbOpts);
