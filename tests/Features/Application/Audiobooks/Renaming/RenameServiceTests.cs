...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.Renaming
{
    public class RenameServiceTests : IDisposable
    {
        private readonly string _tempRoot = Path.Join(Path.GetTempPath(), "BookmarkarrRenameTests", Guid.NewGuid().ToString("N"));
        private readonly List<BookmarkarrDbContext> _contexts = new();
        ...
        private (RenameService Service, BookmarkarrDbContext Db, string DbName) BuildService(
        ...
        private BookmarkarrDbContext CreateContext(string dbName)
        {
            var options = new DbContextOptionsBuilder<BookmarkarrDbContext>()
                .UseInMemoryDatabase(dbName)
                .Options;
            var context = new BookmarkarrDbContext(options);
