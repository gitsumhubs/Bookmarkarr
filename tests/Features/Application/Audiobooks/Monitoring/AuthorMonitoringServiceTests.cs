...
namespace Bookmarkarr.Tests.Features.Application.Audiobooks.Monitoring
{
    public class AuthorMonitoringServiceTests
    {
        ...
        {
            var dbOptions = new DbContextOptionsBuilder<BookmarkarrDbContext>()
                .UseInMemoryDatabase(databaseName: $"author-monitor-{System.Guid.NewGuid():N}")
                .Options;

            await using var dbContext = new BookmarkarrDbContext(dbOptions);
