...
namespace Bookmarkarr.Tests.Features.Application.Downloads.Import
{
    ...
    public class DownloadImportServiceTests : BaseTests
    {
        ...
        [Fact]
        public async Task EbookImport_CopiesRecognizedFormatToSeparateLibrary()
        {
            ...
            var ebookRoot = FileService.GetTempDirectory("ebook-library");
            var sourceRoot = FileService.GetTempDirectory("ebook-download");
            var sourceFile = await FileService.GetFileAsync(sourceRoot, "release.epub");
            var audiobook = await _audiobookRepository.AddAsync(new AudiobookBuilder()
                .WithTitle("Pride and Prejudice")
                .WithAuthor("Jane Austen")
                .WithSeries("Classic Novels")
                .Build());
            ...
            Assert.Equal(expectedPath, results[0].FinalPath);
            Assert.True(File.Exists(expectedPath));
            Assert.True(File.Exists(sourceFile));
            Assert.False(results[0].WasRegisteredToAudiobook);
        ...
        public async Task EbookImport_RejectsAudioOnlyPayload()
        {
            ...
            var ebookRoot = FileService.GetTempDirectory("ebook-library");
            var sourceRoot = FileService.GetTempDirectory("ebook-download");
            var sourceFile = await FileService.GetFileAsync(sourceRoot, "wrong-result.mp3");
            var audiobook = await _audiobookRepository.AddAsync(new AudiobookBuilder()
                .WithTitle("Pride and Prejudice")
                .WithAuthor("Jane Austen")
                .Build());
        ...
        {
            var outputRoot = FileService.GetTempDirectory("bookmarkarr-test-output");
        ...
        public async Task MultiFileImport_ImportsAllFiles_WithUniqueNames()
        {
            ...
            var basePath = FileService.GetTempDirectory("bookmarkarr-multi");
            ...
            var srcDir = FileService.GetTempDirectory("bookmarkarr-src");
        ...
        {
            var outputDir = FileService.GetTempDirectory("bookmarkarr-import-ordered");

            var srcDir = FileService.GetTempDirectory("bookmarkarr-import-ordered-src");
