...
namespace Bookmarkarr.Tests.Features.Application.Downloads.Processing
{
    ...
    public class DownloadProcessingJobServiceTests : BaseTests
    {
        ...
        {
            var sourceDir = FileService.GetTempDirectory("bookmarkarr-pipeline");
            var sourceFile = await FileService.GetFileAsync(sourceDir, "Pipeline Coverage.m4b");

            var outputDir = FileService.GetTempDirectory("bookmarkarr-pipeline-out");
