...
namespace Bookmarkarr.Tests.Features.Application.Downloads.Queue
{
    ...
    public class DownloadQueueServiceReconciliationTests : IDisposable
    {
        ...
        [Fact]
        [Trait("Scenario", "RepeatedTitlePreservesExternalId")]
        public async Task GetQueueAsync_RepeatedTitleHistory_DoesNotRebindEstablishedExternalId()
        {
            var client = new DownloadClientConfiguration
            {
                Id = "nzbget",
            ...
            var processingJobRepoMock = new Mock<IDownloadProcessingJobRepository>();
            processingJobRepoMock.Setup(r => r.GetPendingDownloadIdsAsync(It.IsAny<IEnumerable<string>>()))
                .ReturnsAsync(new List<string>());
            processingJobRepoMock.Setup(r => r.GetAllJobDownloadIdsAsync(It.IsAny<IEnumerable<string>>()))
                .ReturnsAsync(new List<string>());
            ...
            gatewayMock.Setup(g => g.GetQueueAsync(client, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new List<QueueItem>
                {
                    ...
                    {
                        Id = "7725",
                        Title = trackedDownload.Title,
                        Status = "failed",
                        ErrorMessage = "Older attempt failed",
                    ...
                    new()
                    {
                        ...
                        Size = 1_000,
                        Downloaded = 1_000,
                        ContentPath = "/downloads/completed/harry-potter",
                        AddedAt = DateTime.UtcNow
                    }
                });
            ...
            Assert.Equal("tracked-second-attempt", item.Id);
            Assert.Equal("completed", item.Status);
            Assert.Equal("7726", trackedDownload.Metadata?["ClientDownloadId"]?.ToString());
            downloadRepoMock.Verify(
                r => r.UpdateMetadataAsync("tracked-second-attempt", "ClientDownloadId", It.IsAny<object?>()),
                Times.Never);
        }
