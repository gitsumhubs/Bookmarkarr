...
namespace Bookmarkarr.Tests.Features.Api.Startup;

public sealed class BookmarkarrBuilderFactoryTests
{
    [Fact]
    public void EnsureExternalConfiguration_CreatesDefaultConfigurationInsideContentRoot()
    {
        ...
        try
        {
            BookmarkarrBuilderFactory.EnsureExternalConfiguration(contentRoot, new LocalFileSystem());
    ...
    {
        var contentRoot = Path.GetFullPath(Path.Join(Path.GetTempPath(), $"bookmarkarr-startup-{Guid.NewGuid():N}"));
        ...
        BookmarkarrBuilderFactory.EnsureExternalConfiguration(contentRoot, fileSystem.Object);
    ...
    {
        var path = Path.Join(Path.GetTempPath(), $"bookmarkarr-startup-{Guid.NewGuid():N}");
