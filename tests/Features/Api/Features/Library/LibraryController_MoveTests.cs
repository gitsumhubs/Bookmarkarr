...
namespace Bookmarkarr.Tests.Features.Api.Features.Library
{
    ...
    public class LibraryController_MoveTests : BaseTests
    {
        ...
        public async Task MoveAudiobook_EnqueuesJob_WhenSourceExists()
        {
            ...
            var ab = await _audiobookRepository.AddAsync(new AudiobookBuilder()
                .WithTitle("Test")
                .WithBasePath(FileService.GetTempDirectory("bookmarkarr-move-src"))
                .Build());

            var target = Path.Join(FileService.GetTempPath(), "bookmarkarr-move-dst");
        ...
        public async Task MoveAudiobook_UpdatesBasePath_WhenMoveFilesFalse()
        {
            ...
            var ab = await _audiobookRepository.AddAsync(new AudiobookBuilder()
                .WithTitle("Test")
                .WithBasePath(Path.Join(FileService.GetTempPath(), "bookmarkarr-move-src"))
                .Build());
        ...
        public async Task MoveAudiobook_PreservesDestinationPathWhitespace_WhenMoveFilesFalse()
        {
            ...
            var relativeTarget = "  bookmarkarr-move-dst-" + Guid.NewGuid().ToString("N");
            ...
            Assert.StartsWith("  bookmarkarr-move-dst-", Path.GetFileName(updated.BasePath), StringComparison.Ordinal);
        }
        ...
        {
            var outputPath = FileService.GetTempDirectory("bookmarkarr-move-output");
            await _applicationSettingsRepository.SaveAsync(new ApplicationSettingsBuilder()
                .WithOutputPath(outputPath)
                .Build());

            var sourcePath = FileService.GetTempDirectory("bookmarkarr-move-src");
            var audiobook = await _audiobookRepository.AddAsync(new AudiobookBuilder()
                .WithTitle("Test")
