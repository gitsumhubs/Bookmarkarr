...
namespace Bookmarkarr.Tests.Features.Api.Services.Search.Providers
{
  ...
  <channel>
    <item>
      ...
      <enclosure url=""https://example.com/test.torrent"" length=""123456"" />
        ...
        [Fact]
        public async Task ParseTorznabResponse_Preserves_AudiobookBay_Magnet_From_Prowlarr()
        {
            const string magnet = "magnet:?xt=urn:btih:0123456789ABCDEF0123456789ABCDEF01234567&dn=Example%20Book";
            var xml = $$"""
                <?xml version="1.0"?>
                <rss>
                  <channel>
                    <item>
                      <title>Example Author - Example Book [M4B Audiobook]</title>
                      <guid>https://prowlarr.example/api/v1/indexer/7/results/abb-123</guid>
                      <category>Audio/Audiobook</category>
                      <torznab:attr xmlns:torznab="http://torznab.com/schemas/2015/feed" name="category" value="3030" />
                      <torznab:attr xmlns:torznab="http://torznab.com/schemas/2015/feed" name="magneturl" value="{{magnet.Replace("&", "&amp;")}}" />
                    </item>
                  </channel>
                </rss>
                """;
            ...
            {
                Id = 7,
                Name = "AudiobookBay (Prowlarr)",
                Url = "http://prowlarr:9696/7/api",
                Type = "Torrent",
                Implementation = "torznab"
            };

            var results = await CreateSearchService().ParseTorznabResponseAsync(xml, indexer);

            var result = Assert.Single(results);
            Assert.Equal(magnet, result.MagnetLink);
            Assert.Equal(7, result.IndexerId);
            Assert.Equal("torznab", result.IndexerImplementation);
        }
  ...
  <channel>
    ...
    <item>
      ...
      <enclosure url=""https://example.com/test.torrent"" length=""100000"" />
        ...
        {
            var idx = new Bookmarkarr.Domain.Search.IndexerSearchResult
            {
                Id = "1",
                Title = "Test",
                Artist = "Author",
            ...
            var sr = Bookmarkarr.Domain.Search.SearchResultConverters.ToSearchResult(idx);
        ...
        public void SearchIndexers_Appends_ExtendedParameter_For_Torznab_And_Newznab()
        {
            ...
            using var db = new BookmarkarrDbContext(options);
...
""";
            ...
            var json = @"[
  {
    ...
    ""fileName"": ""Frank Herbert - Collection by Frank Herbert [ENG / MP3] [VIP].torrent""
            ...
            var dto = Bookmarkarr.Domain.Search.SearchResultConverters.ToIndexerResultDto(r);
