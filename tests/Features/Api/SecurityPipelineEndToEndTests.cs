...
{
    public class SecurityPipelineEndToEndTests : IClassFixture<BookmarkarrWebApplicationFactory>
    {
        private readonly BookmarkarrWebApplicationFactory _factory;

        public SecurityPipelineEndToEndTests(BookmarkarrWebApplicationFactory factory)
        {
            _factory = factory;
        }
        ...
        public async Task SessionCookie_Authenticates_AndCsrfProtectsUnsafeMethods()
        {
            ...
            unsafeWithoutCsrf.Headers.Add("Cookie", $"bookmarkarr_session={sessionToken}");
            ...
            unsafeWithCsrf.Headers.Add("Cookie", $"bookmarkarr_session={sessionToken}; {antiforgeryCookie}");
        ...
        public async Task SignalRHub_AllowsSessionCookie_WhenAuthEnabled()
        {
            using var factory = CreateFactory(authenticationRequired: "true", apiKey: "server-api-key");
            var sessionToken = await CreateSessionAsync(factory, "admin-user", isAdmin: true);
            await using var connection = CreateHubConnection(factory, cookieHeader: $"bookmarkarr_session={sessionToken}");
