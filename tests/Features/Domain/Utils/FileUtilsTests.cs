...
namespace Bookmarkarr.Tests.Features.Domain.Utils
{
    public class FileUtilsTests
    {
        [Theory]
        [InlineData("book.epub", true)]
        [InlineData("book.AZW3", true)]
        [InlineData("book.pdf", true)]
        [InlineData("book.mp3", false)]
        [InlineData("comic.cbz", false)]
        [InlineData("magazine.cbr", false)]
        public void IsEbookFile_UsesStrictFormatAllowlist(string path, bool expected)
        {
            Assert.Equal(expected, FileUtils.IsEbookFile(path));
        }
        ...
        [InlineData(@"C:\Program Files\Bookmarkarr", false)]
