...
namespace Bookmarkarr.Tests.Mocks
{
    public class BookmarkarrWebApplicationFactory : WebApplicationFactory<Program>
    {
        ...
        public BookmarkarrWebApplicationFactory()
        {
            Environment.SetEnvironmentVariable("BOOKMARKARR_TEST_MODE", "true");
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureAppConfiguration((context, config) =>
            {
                ...
                {
                    _sqliteDbRootDir = Path.Join(Path.GetTempPath(), "bookmarkarr-tests", $"host-{Guid.NewGuid():N}");
                    Directory.CreateDirectory(_sqliteDbRootDir);
                    _sqliteDbPath = Path.Join(_sqliteDbRootDir, "bookmarkarr.db");
                    RegisterDbPathForCleanup(_sqliteDbPath);
                }

                var overrides = new Dictionary<string, string?>
                {
                    ["Bookmarkarr:SqliteDbPath"] = _sqliteDbPath,
