...
Bookmarkarr uses a small set of shared checks so local hooks and CI describe the same expectations.
...
- C# formatting is checked with `dotnet format bookmarkarr.slnx --no-restore --verify-no-changes`.
