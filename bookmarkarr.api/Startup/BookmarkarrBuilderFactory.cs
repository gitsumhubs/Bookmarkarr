...
public static class BookmarkarrBuilderFactory
{
        ...
        IFileSystem fileSystem)
    {
        ...
        {
            var testContentRootPath = Path.Combine(Path.GetTempPath(), "BookmarkarrTests");
            fileSystem.CreateDirectory(testContentRootPath);
            Environment.SetEnvironmentVariable("BOOKMARKARR_CONTENT_ROOT", testContentRootPath);
    ...
    private static string ResolveEnvironmentName()
    {
        ...
        if (string.Equals(processName, "testhost", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(Environment.GetEnvironmentVariable("BOOKMARKARR_TEST_MODE"), "true", StringComparison.OrdinalIgnoreCase) ||
    ...
    {
        var contentRootOverride = Environment.GetEnvironmentVariable("BOOKMARKARR_CONTENT_ROOT");
        ...
        try
        {
            ...
            {
                Console.WriteLine($"[Bookmarkarr] BOOKMARKARR_CONTENT_ROOT '{contentRootOverride}' does not exist; creating it");
        ...
        {
            Console.WriteLine($"[Bookmarkarr] Error: BOOKMARKARR_CONTENT_ROOT '{contentRootOverride}' cannot be used or created; ignoring override.");
            return contentRootPath;
        }
    }
    ...
    internal static void EnsureExternalConfiguration(string contentRootPath, IFileSystem fileSystem)
    {
        ...
        try
        {
            ...
            if (!fileSystem.FileExists(externalConfigAbsolute))
            {
                ...
                Console.WriteLine($"[Bookmarkarr] Created default configuration at '{safeExternalConfigAbsolute}'. Edit this file to customize app settings.");
            }
        }
        ...
        {
            Console.WriteLine($"[Bookmarkarr] Warning: failed to create default config '{externalConfigRelative}': {ex.Message}");
        }
    }

    private static void ConfigureSerilog(WebApplicationBuilder builder, ILogEventSink realtimeLogSink)
    {
        var logFilePath = Path.Join(builder.Environment.ContentRootPath, "config", "logs", "bookmarkarr-.log");
        var logLevelEnv = Environment.GetEnvironmentVariable("BOOKMARKARR_LOG_LEVEL");
        ...
        Log.Logger = new LoggerConfiguration()
            ...
            .Enrich.WithProperty("Application", "Bookmarkarr.Api")
